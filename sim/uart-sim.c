/* From microwatt https://github.com/antonblanchard/microwatt */
...
static unsigned long long global_tick_count = 0;
void uart_tx(int tx)
{
	global_tick_count++;
	switch (tx_state) {
		...
		case START_BIT:
			...
			if (tx == 1) {
				if (is_error(tx_countbits)) {
					printf("START_BIT error %ld %ld %lld\n", BITWIDTH, tx_countbits, global_tick_count);
